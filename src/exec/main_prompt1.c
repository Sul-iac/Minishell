...
/*   Updated: 2024/11/10 19:24:12 by qbarron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char    *ft_get_status_color(void)
{
    if (g_global_sig == 0)
        return (GREEN);
    return (RED);
...
static void    ft_fill_prompt(char *prompt, char *user, char *dir)
{
    char    *first_line;
    char    *second_line;
    char    *color;
    ...
    if (first_line && second_line)
    {
        ...
        ft_strlcat(prompt, second_line, ft_strlen(prompt) + ft_strlen(second_line) + 1);
    }
    free(first_line);
    free(second_line);
}
